(ns irs.main
  "Kotodama WASM entrypoint for the Irs clean-room actor (L4) — Clojure port.
  ...
  facts (`irs.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Filing"       :plural "filings"       :id-prefix "irs_fil"
    :fields [:filerId :form :period :status]      :required [:form :period]
    :coerce {}                       :refs {}}
   {:entity "Entity"       :plural "entities"      :id-prefix "irs_ent"
    :fields [:name :ein :jurisdiction]            :required [:name :ein]
    :coerce {}                       :refs {}}
   {:entity "Dataset"      :plural "datasets"      :id-prefix "irs_dat"
    :fields [:name :agency :updatedAt]            :required [:name :agency]
    :coerce {}                       :refs {}}
   {:entity "Application"  :plural "applications" :id-prefix "irs_app"
    :fields [:applicantId :program :status]       :required [:program :status]
    :coerce {}                       :refs {}}
   {:entity "PublicRecord" :plural "publicrecords" :id-prefix "irs_pub"
    :fields [:recordType :jurisdiction :recordedAt] :required [:recordType :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Notice"       :plural "notices"       :id-prefix "irs_not"
...
(defn emit-facts
  "EAVT facts for one record: {\"irs.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "irs-compat" :tier tier :entities entities} 200])
